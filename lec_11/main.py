...
if response.status_code == 200:
    ...
    posts_filtered_by_title = [post for post in posts if len(post['title'].split()) <= 6]
    print('Posts filtered by title')
    print(json.dumps(posts_filtered_by_title, indent=2))

    posts_filtered_by_body = [post for post in posts if len(post['body'].split('\n')) <= 3]
    print('Posts filtered by body')
    print(json.dumps(posts_filtered_by_body, indent=2))
